...
public class MainActivity extends AppCompatActivity implements GoogleApiClient.ConnectionCallbacks,
    ...
    ImageView imv_timkiem, imv_tranthai,imv_menu;
    String vitriht;
...
//    }
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()) {
            case R.id.menu_ngaymai:
                ev_ngaymai();
                return true;
            case R.id.menu_ngaytieptheo:
                chuyentrang();
                return true;
    ...
    public void loadnextdate(String tp) {
        RequestQueue requestQueue = Volley.newRequestQueue(MainActivity.this);
        String url = "https://api.openweathermap.org/data/2.5/forecast?q=" + tp + "&lang=vi&appid=59952d62b2db0ba8108e2e11126cde00";
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                try {
                    JSONObject object = new JSONObject(response);
                    JSONObject objc = object.getJSONObject("city");
                    String name = objc.getString("name");
                    ...
                    tv_trangthai.setText(des);
                    String icon = objwt.getString("icon");
